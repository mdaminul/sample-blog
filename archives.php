		...
		<?php 
			if(!isset($_REQUEST['date'])) {
				...
				$date1 = $_REQUEST['date'];
				$month1 = substr($_REQUEST['date'],0,2);
				$year1 = substr($_REQUEST['date'],3,4);
			}
		...
		<?php include('header.php') ;?>
			...
			$statement = $db->prepare("SELECT * FROM tbl_post where post_month=? and post_year=? ORDER BY post_id DESC");
			...
			$statement = $db->prepare("SELECT * FROM tbl_post where post_month=? and post_year=? ORDER BY post_id DESC LIMIT $start, $limit");
			$statement->execute(array($month1,$year1));
