				...
				foreach($result as $row)
					{
						...
						<div class="post">
							...
							<p class="comments">
							Comments - <a href="index2.php?id=<?php echo $row['post_id'];?>">
							<?php
								$statement1 = $db->prepare("SELECT * FROM tbl_comments where post_id=?");
								$statement1->execute(array($row['post_id']));
								$total_comments = $statement1->rowCount();
								echo $total_comments;
							?>
							</a>   <span>|</span>   <a href="index2.php?id=<?php echo $row['post_id'];?>">Continue Reading</a></p>
						</div>
						<?php
					}
			?>
